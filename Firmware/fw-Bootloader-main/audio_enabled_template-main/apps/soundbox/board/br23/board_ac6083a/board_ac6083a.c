...
#define LED_CNT_BASE	5//闪灯频率控制


int cur_led_mode=FAST_BLINK;//闪灯状态
static u16 music_turnoff_timer=0;
static u16 led_blink_timer1=0;
static u16 led_blink_timer2=0;
static u16 power_on_timer=0;

static u32 ot;//pc状态区分
static u16 cr_t_id=0;

static u16 led_scan_timer;
static soft_poweroff_timer;
...
static void poweroff_led_blink1()
{
	if(get_charge_online_flag()==0)
		cur_led_mode=FAST_BLINK;
	else
		cur_led_mode=TURN_OFF;
}
...
static void pc_led_handler()
{
	if(!cr_t_id)
		cr_t_id=sys_timer_add(NULL,card_reader_timer,100);
	ot=3;
}

static void soft_poweroff()
{
	int poweroff_pin_value=gpio_read(IO_PORTA_01);
	if(poweroff_pin_value==0){
...
void led_scan()
{
	...
	/* gpio_set_output_value(IO_PORT_DP,1); */
	static u16 cnt;
	switch(cur_led_mode){
		case TURN_OFF:
			led_close();
		...
		case TURN_ON:
			...
			break;
		case FAST_BLINK:
			if(cnt>LED_CNT_BASE*25/2)//变频
				led_open();
			else
				led_close();
			cnt++;
			if(cnt>LED_CNT_BASE*25)
				cnt=0;
			break;
		case SLOW_BLINK:
			if(cnt>LED_CNT_BASE*250/2){
				led_open();
			}
			else{
				led_close();
			...
			cnt++;
			if(cnt>LED_CNT_BASE*250)
				cnt=0;
			break;
		default:
			break;
	}
	/* gpio_set_output_value(IO_PORT_DP,0); */
}

void app_status_6083_handler(enum APP_STATUS status)
{
	switch(status){
		case APP_STATUS_POWER_ON:
			if(cur_led_mode==FAST_BLINK)
				cur_led_mode=TURN_ON;
			break;
		case APP_STATUS_POWER_CLOSE:
			cur_led_mode=TURN_OFF;
			break;
		case APP_STATUS_POWER_OFF:
			cur_led_mode=FAST_BLINK;
			break;
		case APP_STATUS_MUSIC:
			cur_led_mode=SLOW_BLINK;
			break;
		case APP_STATUS_MUSIC_QUIT:
			cur_led_mode=TURN_ON;
			break;
		case APP_STATUS_MUSIC_PP:
			cur_led_mode=TURN_ON;
			music_turnoff_timer=sys_timeout_add(NULL,music_turnoff,30000);
