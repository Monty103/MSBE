...
int audio_encoder_task_open(void)
{
    ...
    if (!encode_task) {
        ...
        int ct = audio_encoder_task_create(encode_task, "audio_enc");
        printf("ENC TASK CREATE %d", ct);
    }
...
#endif
#endif//TCFG_MIC_EFFECT_ENABLE
        printf("ZALLOC %d", ladc_var.ladc_ch_num * irq_point_unit * 2 * LADC_BUF_NUM);
