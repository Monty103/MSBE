...
static void audio_adc1_output_demo(void *priv, s16 *data, int len)
{
  /*  struct audio_adc_hdl *hdl = priv;
    ...
    memcpy(mic_buff, data, 128);
    // printf("linein:%x,len:%d,ch:%d",data,len,hdl->channel);
    for (uint32_t i = 0; i < 128; i++) {
        //mic_buff[i] -= 875;
        printf("%d", mic_buff[i]);
        //mic_buff[i] <<= 1; // amplify input signal
    }
    g726_encode(&g726_encoder_state, g726coded, mic_buff, 128);
    if(start_mic)
    {
      USB_MicSend(g726coded, 64);
     // USB_MicSend(&g726coded[32], 32);
    }
*/
