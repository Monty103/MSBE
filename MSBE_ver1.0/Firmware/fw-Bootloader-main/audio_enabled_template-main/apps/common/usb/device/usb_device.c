...
static void usb_device_init(const usb_dev usb_id)
{
   ...
   /* usb_set_dma_raddr(usb_id, 1, usb_get_ep_buffer(usb_id, 1));
    usb_set_dma_raddr(usb_id, 2, usb_get_ep_buffer(usb_id, 2));
    usb_set_dma_raddr(usb_id, 3, usb_get_ep_buffer(usb_id, 3));
    usb_set_dma_raddr(usb_id, 4, usb_get_ep_buffer(usb_id, 4));*/

    /*usb_set_dma_raddr(usb_id, 0x81, usb_get_ep_buffer(usb_id, 0x81));
    usb_set_dma_raddr(usb_id, 0x82, usb_get_ep_buffer(usb_id, 0x82));
    usb_set_dma_raddr(usb_id, 0x83, usb_get_ep_buffer(usb_id, 0x83));
    usb_set_dma_raddr(usb_id, 0x84, usb_get_ep_buffer(usb_id, 0x84));*/
    ...
    usb_set_intr_txe(usb_id, 1);
    usb_set_intr_rxe(usb_id, 1);

    usb_set_intr_txe(usb_id, 2);
    usb_set_intr_rxe(usb_id, 2);

    usb_set_intr_txe(usb_id, 3);
    usb_set_intr_rxe(usb_id, 3);

    usb_set_intr_txe(usb_id, 4);
    usb_set_intr_rxe(usb_id, 4);
...
int usb_device_mode(const usb_dev usb_id, const u32 class)
{
    ...
    if ((class & HID_CLASS) == HID_CLASS)
    {
        ...
        printf("add desc std hid");
        usb_add_desc_config(usb_id, 0, hid_desc_config0);
        usb_add_desc_config(usb_id, 1, hid_desc_config1);
        usb_add_desc_config(usb_id, 2, hid_desc_config2);
        usb_add_desc_config(usb_id, 3, hid_desc_config3);
         setted=1;
         }
